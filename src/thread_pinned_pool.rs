//! # Thread-Pinned Pool
//!
//! **Zero-contention isolate pool with thread-local storage.**
//!
//! ## Architecture
//!
//! Unlike the Shared Pool which uses a global mutex-protected LRU cache,
//! the Thread-Pinned Pool gives each thread its own local pool:
//!
//! ```text
//! Shared Pool (contention):          Thread-Pinned Pool (no contention):
...
//!     ↑    ↑    ↑                     │[i,i]│   │[i,i]│   │[i,i]│
//!   Th1  Th2  Th3                     └─────┘   └─────┘   └─────┘
//!  (mutex contention)                 (zero contention)
//! ```
//!
//! ## Key Benefits
//!
//! 1. **Zero contention** - No mutex for pool access
//! 2. **Better cache locality** - Isolate stays on same CPU core
//! 3. **Predictable latency** - No spin-wait or lock contention
//! 4. **Security** - With sticky routing, each tenant gets dedicated isolate
//!
//! ## When to Use
//!
//! - Multi-tenant environments (security isolation)
//! - CPU-bound workloads (high contention in shared pool)
//! - When predictable latency is critical (SLA requirements)
//!
//! ## Sticky Routing
//!
//! For security, use sticky routing so same worker always goes to same thread:
//! ```ignore
//! let thread_id = hash(worker_id) % num_threads;
//! // Route request to thread_id
//! ```
