...
fn test_locker_static_methods() {
    ...
    // Test that static methods don't panic when called
    // (they might return false if no locker is active, which is fine)
    let _is_active = v8::Locker::is_active();

    // Success - static methods are callable
}
